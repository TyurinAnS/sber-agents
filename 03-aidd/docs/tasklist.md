...
- [ ] Добавить в `config.py` загрузку `OPENROUTER_API_KEY` и `LLM_MODEL` из `.env`.
- [ ] Реализовать в `llm_service.py` функцию для отправки запроса в LLM через `openai` клиент, используя Openrouter.
- [ ] Добавить обработчик текстовых сообщений в `handlers.py`.
- [ ] В обработчике текстовых сообщений вызывать `llm_service.py` для получения ответа от LLM.
- [ ] Отправлять полученный ответ от LLM пользователю.
- [ ] **Тестирование:** Запустить бота, отправить любой вопрос и убедиться, что LLM отвечает.
