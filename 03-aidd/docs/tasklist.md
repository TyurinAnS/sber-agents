...
| 3        | Контекст диалога (история)      | ✅ Завершено  | ✅           |
| 4        | Конфигурирование                | ✅ Завершено  | ✅           |
| 5        | Логгирование                    | ✅ Завершено  | ✅           |
...
- [x] В `config.py` добавить `LLM_SYSTEM_PROMPT` и `CHAT_HISTORY_LENGTH`.
- [x] Реализовать механизм хранения истории диалога в оперативной памяти (например, словарь в `main.py` или `handlers.py`).
- [x] Модифицировать обработчики сообщений для сохранения истории.
- [x] Модифицировать `llm_service.py` для включения системного промпта и истории последних `CHAT_HISTORY_LENGTH` сообщений в запрос к LLM.
- [x] **Тестирование:** Запустить бота, вести диалог, убедиться, что LLM помнит предыдущие реплики.

### Итерация 4: Доработка конфигурирования

- [x] Добавить более надежную обработку отсутствующих переменных окружения в `config.py` (например, с выдачей ошибок).
- [x] **Тестирование:** Удалить некоторые переменные из `.env` и убедиться, что приложение корректно сообщает об ошибках.

### Итерация 5: Базовое логгирование

- [x] Настроить базовое логгирование с использованием `logging` в `main.py`.
- [x] Добавить информационные логи о старте бота, получении сообщений, отправке запросов/ответов LLM.
- [x] Добавить логи ошибок при сбоях.
- [x] **Тестирование:** Запустить бота, взаимодействовать с ним, вызвать ошибку (например, неверный API ключ) и проверить вывод логов в консоль.
