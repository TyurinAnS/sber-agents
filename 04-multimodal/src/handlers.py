...
from llm import get_transaction_response_text, get_transaction_response_image, transcribe_voice
...
async def handle_voice(message: Message):
    ...
    voice = message.voice
    logger.info(f"Voice message from {chat_id}, duration: {voice.duration}s, size: {voice.file_size} bytes")
    ...
    try:
        # Уведомляем пользователя о начале транскрибации
        ...
        logger.info(f"Downloaded voice message: {len(audio_bytes)} bytes")
        
        # Транскрибация
        text = await transcribe_voice(audio_bytes)
        
        logger.info(f"Transcribed voice from {chat_id}: '{text[:100]}...'")
        
        # Удаляем сообщение о транскрибации
        await status_msg.delete()
        
        # Обрабатываем транскрибированный текст напрямую
        # (не можем изменить message.text т.к. объект frozen)
        ...
        if response.transactions:
            ...
            logger.info(f"Extracted {len(response.transactions)} transactions from voice for {chat_id}")
        ...
        chat_conversations[chat_id].append(
            {"role": "user", "content": f"[Голосовое]: {text}"}
        )
        ...
        logger.error(f"Error handling voice from {chat_id}: {e}", exc_info=True)
        await message.answer(
