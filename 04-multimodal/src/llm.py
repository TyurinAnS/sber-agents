...
# Инициализация модели Faster Whisper для транскрибации
# Используем модель "base" как оптимальный баланс между скоростью и качеством
logger.info("Loading Faster Whisper model...")
whisper_model = WhisperModel(
    "base",  # оптимальная модель для CPU (баланс скорость/качество)
...
async def transcribe_voice(audio_bytes: bytes) -> str:
    """
    Транскрибация голосового сообщения через Faster Whisper локально.
    
    Args:
        audio_bytes: Байты аудиофайла (OGG Opus из Telegram)
        
    Returns:
        Транскрибированный текст
        
    Raises:
        Exception: При ошибке транскрибации
    """
    try:
        ...
        logger.info(f"Transcribing voice message, file size: {len(audio_bytes)} bytes")
        
        try:
            # Транскрибация в отдельном потоке (whisper работает синхронно)
            def transcribe_sync():
                segments, info = whisper_model.transcribe(
                    temp_path,
                    language="ru",  # русский язык
                    beam_size=5,  # баланс между качеством и скоростью
            ...
            # Запускаем в executor для неблокирующей работы в async контексте
            text = await asyncio.get_event_loop().run_in_executor(None, transcribe_sync)
            
            logger.info(f"Transcribed text (length {len(text)}): {text[:200]}...")
            
            if not text:
                logger.warning("Transcription resulted in empty text")
                raise ValueError("Не удалось распознать речь в голосовом сообщении")
