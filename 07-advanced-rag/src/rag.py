...
def _get_llm_query_transform():
    ...
    if _llm_query_transform is None:
        ...
        if config.OPENAI_API_KEY:
            ...
            logger.info(f"Query transform LLM initialized (OpenAI): {config.MODEL_QUERY_TRANSFORM}")
        else:
            ...
            logger.info(f"Query transform LLM initialized (Ollama): {config.MODEL_QUERY_TRANSFORM}")
...
def _get_llm():
    ...
    if _llm is None:
        ...
        if config.OPENAI_API_KEY:
            ...
            logger.info(f"Main LLM initialized (OpenAI): {config.MODEL}")
        else:
            _llm = ChatOllama(
                model=config.MODEL,
                temperature=0.9,
            ...
            logger.info(f"Main LLM initialized (Ollama): {config.MODEL}")
    return _llm
