...
def create_ragas_embeddings():
    ...
    if provider == "openai":
        ...
        logger.info(f"Creating RAGAS Ollama embeddings: {config.OLLAMA_EMBEDDING_MODEL} at {config.OLLAMA_BASE_URL}")
...
def init_ragas_metrics():
    ...
    # Если OPENAI_API_KEY установлен - используем OpenAI, иначе - Ollama
    if config.OPENAI_API_KEY:
        langchain_llm = ChatOpenAI(model=config.RAGAS_LLM_MODEL, temperature=0)
        logger.info(f"Using OpenAI for RAGAS LLM: {config.RAGAS_LLM_MODEL}")
    else:
        langchain_llm = ChatOllama(model=config.RAGAS_LLM_MODEL, temperature=0, base_url=config.OLLAMA_BASE_URL)
        logger.info(f"Using Ollama for RAGAS LLM: {config.RAGAS_LLM_MODEL}")
    ...
    # Настройки для выполнения (увеличены таймауты для медленных локальных моделей)
    run_config = RunConfig(
        max_workers=2,  # Уменьшаем параллельность для стабильности
        max_wait=300,   # Увеличиваем таймаут до 5 минут
    ...
    if config.RAGAS_EMBEDDING_PROVIDER == "openai":
        ...
        logger.info(f"✓ RAGAS Embedding Model: {config.OLLAMA_EMBEDDING_MODEL} at {config.OLLAMA_BASE_URL}")
    else:
